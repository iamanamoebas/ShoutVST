...
BE_ERR beInitStream(PBE_CONFIG pbeConfig, PDWORD dwSamples, PDWORD dwBufferSize,
  ...
  *phbeStream = (HBE_STREAM)gfp;
  ...
  if (pbeConfig->dwConfig != BE_CONFIG_LAME) {
    ...
    // sound quality.  Forcing output samplerate = input samplerate is not a
    // good idea
  ...
  // The following settings only used when preset is not one of the LAME QUALITY
  // Presets
  if ((int)lameConfig.format.LHV1.nPreset < (int)LQP_STANDARD) {
    switch (lameConfig.format.LHV1.nMode) {
      ...
      case BE_MP3_MODE_MONO:
        ...
        break;
      case BE_MP3_MODE_DUALCHANNEL:  // warning: there is NO dual channel option
        // working in Lame
    ...
    */
    ...
    /* Set bitrate.  (CDex users always specify bitrate=Min bitrate when using
 * VBR) */
...
*/
  ...
  // Override Quality setting, use HIGHBYTE = NOT LOWBYTE to be backwards
  // compatible
  ...
  *dwBufferSize =
      (DWORD)(1.25 * (*dwSamples / lame_get_num_channels(gfp)) + 7200);
...
BE_ERR beEncodeChunk(HBE_STREAM hbeStream, DWORD nSamples, PSHORT pSamples,
                     PBYTE pOutput, PDWORD pdwOutput) {
  // Encode it
  int dwSamples;
  int nOutputSamples = 0;
  lame_global_flags* gfp = (lame_global_flags*)hbeStream;
  ...
  if (1 == lame_get_num_channels(gfp)) {
    nOutputSamples =
        lame_encode_buffer(gfp, pSamples, pSamples, dwSamples, pOutput, 0);
  } else {
    nOutputSamples =
        lame_encode_buffer_interleaved(gfp, pSamples, dwSamples, pOutput, 0);
  }
...
BE_ERR beEncodeChunkFloatS16NI(HBE_STREAM hbeStream, DWORD nSamples,
  ...
  nOutputSamples =
      lame_encode_buffer_float(gfp, buffer_l, buffer_r, nSamples, pOutput, 0);
...
static int maybeSyncWord(FILE* fpStream) {
  unsigned char mp3_frame_header[4];
  size_t nbytes =
      fread(mp3_frame_header, 1, sizeof(mp3_frame_header), fpStream);
...
static int skipId3v2(FILE* fpStream, size_t lametag_frame_size) {
  size_t nbytes;
...
static BE_ERR updateLameTagFrame(lame_global_flags* gfp, FILE* fpStream) {
