...
typedef struct {
  ...
  union {
    struct {
      ...
      // BE_MP3_MODE_MONO
      WORD wBitrate;  // 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224,
      // 256 and 320 allowed
      BOOL bPrivate;
      BOOL bCRC;
      BOOL bCopyright;
    ...
    struct {
      ...
      LONG nMode;            // BE_MP3_MODE_STEREO, BE_MP3_MODE_DUALCHANNEL,
      // BE_MP3_MODE_MONO
      DWORD dwBitrate;     // CBR bitrate, VBR min bitrate
      DWORD dwMaxBitrate;  // CBR ignored, VBR Max bitrate
      LONG nPreset;        // Quality preset, use one of the settings of the
      // LAME_QUALITY_PRESET enum
      ...
      WORD nQuality;    // Quality Setting, HIGH BYTE should be NOT LOW byte,
      // otherwhise quality=5
